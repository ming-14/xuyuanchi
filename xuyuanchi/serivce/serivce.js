...
{
    return (account === config.AdminAccount && password === config.AdminPassword);
}

app.all("*", function (req, res, next) {
    res.setHeader("Access-Control-Allow-Origin", "*"); // Your website address
...
app.post('/api/deleteABox/:id', (req, res) => {
    if(!isAdmin(req.params.account, req.params.password)) 
    {
        res.json({ code: 403 });
        return;
    }
...
app.listen(config.port, () => {
    console.log(`服务器已打开, 可以运行 http://127.0.0.1:${config.port}`);
});